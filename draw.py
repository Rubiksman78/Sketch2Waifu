...
value = DRAW_MASK
eraser_mode = False
THICKNESS = -1  # solid color_domain circle
...
def process(cmap,edge,gen):
    cmap = torch.from_numpy(cmap.transpose(2,0,1)).squeeze().to(device)
    edge = torch.from_numpy(edge).unsqueeze(0).to(device)
    res = torch.cat((edge,cmap),dim=0)
    res = torch.unsqueeze(res,0).float()
